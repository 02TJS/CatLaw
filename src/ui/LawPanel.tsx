...
export function LawPanel({ controller }: { controller: GameController }) {
  ...
  return <div className="law-panel">
    ...
    <div className="quick-laws">
      ...
      <button onClick={() => setText("有唯一生产计划时优先完成计划；缺料时继续选择不占用计划物资的盈利制作")}>单计划优先</button>
    ...
    {draft && <section className="draft-card" data-testid="law-draft">
      ...
      <p><strong>一句话概述：</strong>{draft.summary}</p>
      ...
      <div className="law-explanation" data-testid="law-explanation">
        ...
        <div>{draft.explanation?.trim() || draft.summary}</div>
      </div>
      <div className="speech-preview"><strong>DeepSeek 匹配法规生成的猫咪台词</strong><ul>{safeSpeechTemplates(draft.speechTemplates).map((line, index) => <li key={`${index}-${line}`}>{line}</li>)}</ul></div>
      <details><summary>查看带逐函数参数注释的不可变源码</summary><pre className="law-source">{draft.sourceCode}</pre></details>
    ...
    <div className="law-list">
      {state.laws.map((law, index) => <article className={`law-card ${law.status}`} key={law.id} data-testid={`law-${law.id}`}>
        ...
        <div className="law-card-main">
          ...
          <details><summary>查看原文、完整解释、台词与注释源码</summary><p>{law.playerText}</p><div className="law-explanation compact"><strong>DeepSeek 白话解释（完整）</strong><div>{law.explanation?.trim() || law.summary}</div></div><div className="speech-preview compact"><strong>决策台词</strong><ul>{safeSpeechTemplates(law.speechTemplates).map((line, speechIndex) => <li key={`${speechIndex}-${line}`}>{line}</li>)}</ul></div><pre className="law-source">{law.sourceCode}</pre><small>{law.astHash}</small></details>
        </div>
